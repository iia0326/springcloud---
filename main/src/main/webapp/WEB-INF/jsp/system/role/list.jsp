...
<%--${page}--%>
    ...
    <script type="text/html" id="btns">
        <shiro:hasPermission name="permission:update">
            <a lay-event="edit" href="role/edit/page?id={{d.id}}" class="layui-btn layui-btn-warm layui-btn-xs">修改</a>
        </shiro:hasPermission>
        <shiro:hasPermission name="permission:delete">
            <button lay-event="del" class="layui-btn layui-btn-danger layui-btn-xs">删除</button>
        </shiro:hasPermission>
        <shiro:hasPermission name="permission:set-menu">
            <a lay-event="edit" href="role/bind/menu/page?id={{d.id}}" class="layui-btn layui-btn-normal layui-btn-xs">绑定菜单</a>
        </shiro:hasPermission>
        <shiro:hasPermission name="permission:set-permission">
            <a lay-event="edit" href="role/bind/permission/page?id={{d.id}}" class="layui-btn layui-btn-primary layui-btn-xs">绑定权限</a>
        </shiro:hasPermission>
    </script>
...
<%--            </div>--%>
            <div class="layui-inline">
                ...
                <shiro:hasPermission name="permission:insert">
                    <a href="role/add/page" class="layui-btn layui-btn-sm" lay-event="add">添加</a>
                </shiro:hasPermission>
            </div>
        </form>
    </script>
            ...
            table.on('tool(test)',function(obj){
                ...
                if(obj.event == 'del'){
                    layer.confirm('正在删除',{
                        ...
                        console.log('confirm')
                        location.href = '<%=basePath%>role/delete?id='+id
                        layer.close(index)
                    })
                }
            })
